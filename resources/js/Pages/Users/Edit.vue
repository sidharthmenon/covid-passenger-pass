        ...
        .col-sm-8
          form.card(@submit.prevent="submit")
            .card-body.border-bottom.p-5
              ...
              form_input(type="password", name="password", label="Password", placeholder="Enter New Password/ Leave blank for exiting password" v-model="form.password")
...
export default {
  ...
  methods:{
    submit(){
      this.sending = true
      this.$inertia.put(this.route('users.update', {id: this.user.id}), {
        name: this.form.name,
